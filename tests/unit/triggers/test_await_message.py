from airflow_provider_openmldb.triggers.await_message import AwaitMessageTrigger
...
def test_trigger_serialization():
    ...
    assert classpath == "airflow_provider_openmldb.triggers.await_message.AwaitMessageTrigger"
    assert kwargs == dict(
        apply_function="test.noop",
        topics="noop",
        apply_function_args=[1, 2],
